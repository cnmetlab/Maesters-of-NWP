...
def get_urls(start_date:datetime, end_date:datetime)->list:
    ...
    """
    dates = pd.date_range(start_date.strftime('%Y-%m-%d %H:00'),end_date.strftime('%Y-%m-%d %H:00'),freq='6H')
    urls = [d.strftime(NCEP_FNL.download_url) if d > datetime(2007,12,6,6) \
        else d.strftime(NCEP_FNL.download_url).replace('/OS/','/').replace('grib2','grib1').replace('http','https') for d in dates]
    return urls
