...
logger.add(os.path.join(os.path.dirname(MAESTERS),'log/DWD_ICON_{time:%Y%m%d}'),rotation='00:00',retention=10)
...
def parse_filename(fn:str):
    ...
    """
    parse_pattern = f'icon_global_[a-z]+_([a-z]+)-[a-z]+_([0-9]+)_([0-9]+)_([0-9A-Z\_]+).grib2*'
    match = re.match(parse_pattern, fn)
    return match


def get_files_dict(date:datetime,hour:int=None,var_dict:dict=DWD_ICON.variable):
    ...
    for variable,out in var_dict.items():
        url = os.path.dirname(DWD_ICON.download_url).format(batch=batch,variable=variable.varname.lower())
        resp = requests.get(url)
        if resp.status_code == 200:
            bs_items= BeautifulSoup(resp.text, 'html.parser')
            files_list = [i['href'] for i in bs_items.find_all('a', text=re.compile(f'icon_global_*'))]
            for f in files_list:
                ...
                if match and int(match[3]) in hours:# HOURS['medium']:
                    ...
                    if level_type == 'single':
                        varname = ''.join(re.findall(r'^[A-Z\d]+|\_[\dA-Z]+|[^\d\_]', match[4]))
                    else:
                        varname = ''.join(re.findall(r'[A-Z]+\_[\dA-Z]+|[^\d\_]+$', match[4]))
                    if V(varname,level_type,level)==variable:
                        res_dict[f'{DWD_ICON.variable.get(variable).outname}-{match[3]}'] = {'url':os.path.join(url,f)}
        else:
...
def download(**kwargs):
    ...
    """
    single_session_download(download_url=kwargs['url'],local_fp=kwargs['local_fp'].replace('.nc','.grib2'),file_type='bz2')
    single_tri_transform(kwargs['local_fp'].replace('.nc','.grib2'),kwargs['local_fp'],os.path.basename(kwargs['local_fp']).split('-')[0])
...
def save_dwd_icon(date:datetime,local_dir:str):
    ...
    with ProcessPoolExecutor(max_workers=PARALLEL_NUM) as pool:
        ...
        for n,r in enumerate(as_completed(results)):
            ...
            if res:
                ...
                logger.info(f'DWD_ICON: [DATE: {date:%Y%m%d} BATCH: {str(batch).zfill(2)} VARNAME: {list(DWD_ICON.variable.keys())[n].varname}] DOWNLOAD FINISH')
    ...
    if fail:
        ...
        logger.info(f'DWD_ICON: [DATE: {date:%Y%m%d} BATCH: {str(batch).zfill(2)}] ALL DOWNLOAD FINISH')
        return 0


def dwd_transform(grib_dir:str,out_dir:str,grid_text:str=os.path.join(MAESTERS,'static/dwd/target_grid_world_0125.txt'),\
    grid_weight:str=os.path.join(MAESTERS,'static/dwd/weights_icogl2world_0125.nc'))->int:
    ...
    for n,m in enumerate(matches):
        if m:
            ...
            if level and level_type and hour:
                ...
                if o:
                    t = (files[n],os.path.join(out_dir,f'{o.outname}-{hour}.nc'),o.outname)
