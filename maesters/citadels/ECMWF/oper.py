...
ECMWF_OPER = get_model('ecmwf','oper')
HOURS = {
...
def operation(data_dir:str=None):
    ...
    tmp_dir = now.strftime(os.path.join(ECMWF_OPER.data_dir.replace('~',os.environ.get('HOME'))+'_tmp',\
        f'%Y%m%d{str(batch).zfill(2)}0000')) if data_dir is None else data_dir+'_tmp'
    data_dir = now.strftime(os.path.join(ECMWF_OPER.data_dir.replace('~',os.environ.get('HOME')), \
